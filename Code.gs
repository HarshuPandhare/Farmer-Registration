...
function rowToObject(row) {
  const obj = {};
  // Google Sheets silently auto-converts date-like strings (e.g. "2024-05-10")
  // into real Date cells. If we hand those straight back, JSON.stringify()
  // turns them into full UTC ISO timestamps (e.g. "2024-05-09T18:30:00.000Z"),
  // which can shift the date back a day depending on timezone and breaks the
  // "yyyy-MM-dd" value the frontend's <input type="date"> fields expect. We
  // normalize any Date cell back to a plain "yyyy-MM-dd" string here, using
  // the spreadsheet's own timezone so the date matches what was entered.
...
function uploadBase64ToDrive(base64String, fileName) {
  try {
    ...
    // NOTE: 'https://drive.google.com/uc?export=view&id=...' (the old URL used
    // here) frequently fails to render inside an <img> tag — Google serves a
    // "can't preview this file" / virus-scan interstitial page instead of the
    // raw image for many files. The "thumbnail" endpoint below reliably
    // returns the actual image bytes and is the recommended way to hotlink a
    // Drive image. sz=w1000 caps the width at 1000px (raise/remove if needed).
